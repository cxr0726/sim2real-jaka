...
from lcm_types.mimic_obs import mimic_obs
import lcm
...
class StateMachine:
    ...
    def update(self, controller_data):
      ...
      #  print(left_key_just_pressed,right_key_just_pressed)
...
class XRobotTeleopToRobot:
    def __init__(self, args):
        ...
        # 1. 实例化消息对象
        self.msg = mimic_obs()
        self.msg.quat_plus_flag=np.array([0.,0.,0.,1.,0.])
        
        self.save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "record_data")
        
        # 2. 如果文件夹不存在，则创建
        os.makedirs(self.save_dir, exist_ok=True)
        print(f"Data recording directory ready: {self.save_dir}")
        
        self.record_episode_idx = 190  # 对应文件名中的 i
        self.recording_buffer = {    # 缓存每一帧的数据
            'root_pos': [],
            'root_rot': [],
            'dof_pos': []
        }
        ...
        # Min link height offset state (migrated from pico_retarget_pub.py)
    ...
    def _apply_min_link_height_offset(
        ...
        """Apply minimum link height offset to correct PICO's Z-axis estimation error.
        
        Migrated from pico_retarget_pub.py. Computes offset between the lowest body Z
        and a target min height, then uniformly shifts all bodies' Z coordinates.
        
        Strategies:
          - 'none': No correction
          - 'per_frame': Independent correction each frame (may cause jitter)
          - 'bootstrap': Calibrate over first N frames, then fix the offset
        """
    ...
    def process_retargeting(self, smplx_data):
        ...
        # Apply min link height offset before retargeting (migrated from pico_retarget_pub.py)
        raw_min_body_z = float(min(
            np.asarray(pose[0], dtype=np.float32)[2] for pose in smplx_data.values()
        ))
        smplx_data = self._apply_min_link_height_offset(smplx_data, raw_min_body_z=raw_min_body_z)
        
        # Retarget with offset_to_ground=False (height correction already done above)
        qpos = self.retarget.retarget(smplx_data, offset_to_ground=False)
        
        # Apply Pelvis Z compensation (migrated from pico_retarget_pub.py)
        processed_human_data = getattr(self.retarget, "scaled_human_data", None)
        if processed_human_data is not None and "Pelvis" in processed_human_data:
    ...
    def handle_state_transitions(self, current_retarget_obs):
        ...
        if current_state == "teleop":
            ...
            if previous_state in ["idle", "pause"]:
                self._start_recording() # 调用开始录制辅助函数
            # ===================================================
            self._handle_enter_teleop(previous_state, current_retarget_obs)

        elif current_state == "pause":
            # ================= NEW: 保存录制逻辑 =================
            if previous_state == "teleop":
                self._save_recording() # 调用保存辅助函数
            # ===================================================
            self._handle_enter_pause()
    ...
    def _save_recording(self):
        ...
        data_to_save = {
            ...
            'root_pos': np.array(self.recording_buffer['root_pos']), # shape (N, 3)
            'root_rot': np.array(self.recording_buffer['root_rot']), # shape (N, 4)
            'dof_pos': np.array(self.recording_buffer['dof_pos'])    # shape (N, 29)
        }
        ...
        try:
            ...
            print(f"--- Saved Episode {self.record_episode_idx} to {filename} (Frames: {num_frames}, FPS: {actual_fps:.2f}) ---")
    ...
    def run(self):
        ...
        with mjv.launch_passive(
            ...
            while viewer.is_running():
                ...
                # ================= NEW: 数据采集逻辑 =================
                # 只有在 teleop 状态且 qpos 有效时才记录
                if self.state_machine.state == "teleop" and qpos is not None:
                    # qpos 结构: [0:3] root_pos, [3:7] root_quat, [7:] joints
                ...
                # self.msg.mimic_obs[:35] = mimic_obs_to_send
                # if self.state_machine.state=='teleop':
                #     self.msg.quat_plus_flag[:4] = qpos[3:7][[1,2,3,0]]
                # elif self.state_machine.state=='idle':
                #     self.state_machine.reset_flag=-1.
                # self.msg.quat_plus_flag[-1] = self.state_machine.reset_flag
                last_qpos=qpos.copy()
            # msg.ang_vel=qvel[:6]f
               # self.lc.publish("MIMIC_OBS", self.msg.encode())
                self.state_machine.reset_flag = 0.
...
def parse_arguments():
    ...
    parser.add_argument(
        "--robot",
        choices=["unitree_g1", "unitree_g1_with_hands","jaka_mini"],
        default="unitree_g1",
    )
    parser.add_argument(
        "--record_video",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Target minimum link height in meters (default: 0.01).",
    )
    parser.add_argument(
        ...
        default="bootstrap",
        help="Strategy for min link height correction: none, per_frame, or bootstrap (default: bootstrap).",
    )
    parser.add_argument(
        "--min_link_height_bootstrap_frames",
        type=int,
        default=30,
        help="Number of frames to use for bootstrap calibration (default: 30).",
    )
